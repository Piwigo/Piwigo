...
{
  if ($upgrade_id >= 171) // TODO change on each release (first task to run)
  {
    break;
  }
  ...
  array_push(
    ...
    array(
      ...
      'description' => '[migration from 14.0.0 to '.PHPWG_VERSION.'] not applied', // TODO change on each release
      )
    );
}
...
for ($upgrade_id = 171; $upgrade_id <= 174; $upgrade_id++) // TODO change on each release
{
...
INSERT INTO `'.PREFIX_TABLE.'upgrade`
  ...
  VALUES
  (\''.$upgrade_id.'\', NOW(), \'[migration from 14.0.0 to '.PHPWG_VERSION.', '.get_elapsed_time($up_start, get_moment()).'] '.$upgrade_description.'\')
;';
  pwg_query($query);
}
